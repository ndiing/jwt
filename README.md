...
const JWT = require('@ndiinginc/jwt')
...
for (let i = 0; i < docs.length; i++) {
    ...
    const data = doc.data;
    const signature = doc.token.split(".")[2];
    const options = { headers, secret };

    var encoded = JWT.encode(data, options);
    // console.log(encoded);
    // var decoded = JWT.decode(encoded, options);
    // console.log(decoded);
    // var sign = JWT.sign(encoded, options);
    // console.log(sign);
    // var signature = encoded.split(".")[2];
    var verified = JWT.verify(encoded, signature, options);
    console.log(alg, verified);
}
